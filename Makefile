.PHONY: install ubuntu-php-cli-fpm-7-3-30 ubuntu-php-cli-fpm-7-2-21 ubuntu-php-cli-fpm-7-1-31 ubuntu-php-cli-fpm-7-0-33 alpine-php-cli-fpm-7-23-8 alpine-php-cli-fpm-7-2-21 alpine-php-cli-fpm-7-1-31 alpine-php-cli-fpm-7-0-33

ubuntu-php-cli-fpm-7-3-30:
	docker image build --build-arg USER_ID=$(shell id -u) --build-arg PHP_VERSION=7.3.30 -t 00f100/magento-php:temp ubuntu;
	mkdir -p $(PWD)/ubuntu/7.3.30/transfer-data;
	mkdir -p $(PWD)/ubuntu/7.3.30/transfer-libs;
	mkdir -p $(PWD)/ubuntu/7.3.30/transfer-libs2;
	docker volume create \
	    --driver local \
	    --opt type=none \
	    --opt device=$(PWD)/ubuntu/7.3.30/transfer-data \
	    --opt o=bind \
	    transfer_data;
	docker volume create \
	    --driver local \
	    --opt type=none \
	    --opt device=$(PWD)/ubuntu/7.3.30/transfer-libs \
	    --opt o=bind \
	    transfer_libs;
	docker volume create \
	    --driver local \
	    --opt type=none \
	    --opt device=$(PWD)/ubuntu/7.3.30/transfer-libs2 \
	    --opt o=bind \
	    transfer_libs2;
	...
	docker build --build-arg USER_ID=$(shell id -u) -t 00f100/magento-php-cli:7.3.30-ubuntu -f ubuntu/7.3.30/cli/Dockerfile ubuntu
	docker build --build-arg USER_ID=$(shell id -u) -t 00f100/magento-php-fpm:7.3.30-ubuntu -f ubuntu/7.3.30/fpm/Dockerfile ubuntu
	...
	sudo rm -Rf $(PWD)/ubuntu/7.3.30/transfer-data;
	sudo rm -Rf $(PWD)/ubuntu/7.3.30/transfer-libs;
	sudo rm -Rf $(PWD)/ubuntu/7.3.30/transfer-libs2;
...
alpine-php-cli-fpm-7-3-30:
	docker image build --build-arg USER_ID=$(shell id -u) --build-arg PHP_VERSION=7.3.30 -t 00f100/magento-php:temp alpine;
	mkdir -p $(PWD)/alpine/7.3.30/transfer-data;
	mkdir -p $(PWD)/alpine/7.3.30/transfer-libs;
	docker volume create \
	    --driver local \
	    --opt type=none \
	    --opt device=$(PWD)/alpine/7.3.30/transfer-data \
	    --opt o=bind \
	    transfer_data;
	docker volume create \
	    --driver local \
	    --opt type=none \
	    --opt device=$(PWD)/alpine/7.3.30/transfer-libs \
	    --opt o=bind \
	    transfer_libs;
	...
	docker build --build-arg USER_ID=$(shell id -u) -t 00f100/magento-php-cli:7.3.30-alpine -f alpine/7.3.30/cli/Dockerfile alpine
	docker build --build-arg USER_ID=$(shell id -u) -t 00f100/magento-php-fpm:7.3.30-alpine -f alpine/7.3.30/fpm/Dockerfile alpine
	docker rmi 00f100/magento-php:temp;
	docker volume rm transfer_data;
	docker volume rm transfer_libs;
	sudo rm -Rf $(PWD)/alpine/7.3.30/transfer-data
	sudo rm -Rf $(PWD)/alpine/7.3.30/transfer-libs
